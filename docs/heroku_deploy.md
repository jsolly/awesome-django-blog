...
2. Check if you're already connected to the app:
   ...
   ```bash
   npx heroku apps:info -a <your_app_name>
   ```

3. If you're not connected, add the remote:

   ```bash
   npx heroku git:remote -a <your_app_name>
   ```
...
```bash
npx heroku config:set DJANGO_SETTINGS_MODULE=app.settings -a <your_app_name>
npx heroku config:set SECRET_KEY=<your-secret-key> -a <your_app_name>
npx heroku config:set DEBUG=False -a <your_app_name>
npx heroku config:set ALLOWED_HOSTS="<your_app_name>.herokuapp.com" -a <your_app_name>
npx heroku config:set USE_SQLITE=False -a <your_app_name>

# AWS S3 settings
npx heroku config:set USE_CLOUD=True -a <your_app_name>
npx heroku config:set AWS_ACCESS_KEY_ID=<your-aws-key> -a <your_app_name>
npx heroku config:set AWS_SECRET_ACCESS_KEY=<your-aws-secret> -a <your_app_name>
npx heroku config:set AWS_STORAGE_BUCKET_NAME=<your-bucket-name> -a <your_app_name>

# Google Gemini
npx heroku config:set GEMINI_API_KEY=<your-gemini-key> -a <your_app_name>
```
...
1. Verify PostgreSQL addon is installed:
   ...
   ```bash
   npx heroku addons:info heroku-postgresql -a <your_app_name>
   ```

2. If not installed, create it:

   ```bash
   npx heroku addons:create heroku-postgresql:mini -a <your_app_name>
   ```
...
4. Run migrations:
   ...
   ```bash
   npx heroku run python manage.py migrate -a <your_app_name>
   ```
...
1. Open your application:
   ...
   ```bash
   npx heroku open -a <your_app_name>
   ```

2. Check logs if needed:

   ```bash
   npx heroku logs --tail -a <your_app_name>
   ```
...
- Verify database URL: `npx heroku config:get DATABASE_URL -a <your_app_name>`
- Check database status: `npx heroku pg:info -a <your_app_name>`

### Static Files Missing

- Ensure AWS S3 credentials are correct
- Verify static files were collected: `npx heroku run python manage.py collectstatic --noinput -a <your_app_name>`

### Application Errors

- Check logs: `npx heroku logs --tail -a <your_app_name>`
- Verify all environment variables are set: `npx heroku config -a <your_app_name>`

## Useful Commands

- Scale dynos: `npx heroku ps:scale web=1 -a <your_app_name>`
- Run Django shell: `npx heroku run python manage.py shell -a <your_app_name>`
- Database shell: `npx heroku pg:psql -a <your_app_name>`
- Restart application: `npx heroku restart -a <your_app_name>`
...
1. Current domains setup:
   ...
   ```bash
   npx heroku domains -a <your_app_name>
   ```
...
2. For new domains, add them using:
   ...
   ```bash
   npx heroku domains:add <your_app_name>.com -a <your_app_name>
   npx heroku domains:add www.<your_app_name>.com -a <your_app_name>
   ```
...
4. Verify DNS configuration:
   ...
   ```bash
   npx heroku domains:wait -a <your_app_name>
   ```

5. Update ALLOWED_HOSTS:

   ```bash
   npx heroku config:set ALLOWED_HOSTS="<your-heroku-domain> <your-custom-domain> www.<your-custom-domain>" -a <your_app_name>
   ```

6. SSL should be automatically enabled, but verify with:

   ```bash
   npx heroku certs:auto -a <your_app_name>
   ```
