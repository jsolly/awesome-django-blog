...
- **Fail loud.** "Done" is wrong if anything was skipped silently; "tests pass" is wrong if any were skipped, excluded, or never run. Surface partial completion and uncertainty instead of papering over it.
...
- For personal projects, use `/review-fix-push-babysit` to review changes, fix issues, commit, push, babysit CI until green, and run required deploys. (No PR step.) In app repos, that skill runs an explicit **fleet freshness gate** (step 1a) before its first commit — syncing stale `.agents/FLEET.lock` via `./scripts/cloud-fleet-sync-if-stale.sh` when needed. Hooks remain fail-only and never auto-sync.
...
- **Cursor discovery:** skills live under `.agents/skills/`, review prompts under `.agents/agents/`, and guideline rules under `.agents/rules/`; `converge-repo.sh` wires all three into `.cursor/skills/`, `.cursor/agents/`, and `.cursor/rules/*.mdc` via `.agents/scripts/link-fleet-cursor.sh`. Project-only discovery files stay as real files in `.cursor/`.
