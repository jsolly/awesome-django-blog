...
| --- | --- | --- |
| `{PLAN_OR_SPEC}` | Most recently modified `<repo-root>/docs/superpowers/plans/*.md` within the session, falling back to `<repo-root>/docs/superpowers/specs/*.md`, or a path referenced explicitly in recent messages. See `references/orchestration.md` step 3 and `.agents/rules/specs-and-plans.md`. | Use the conversation's first user message describing intent. If none, write `No explicit plan or spec — review against the diff and project guidelines only.` |
| `{ARCHITECTURAL_NOTES}` | The orchestrator's notes from step 5 (architectural sanity check) | `No architectural concerns noted.` |
| `{GUIDELINES}` | Concatenated content of project root `AGENTS.md`, any `AGENTS.md` in directories containing modified files, and `.agents/AGENTS.md` | The user's global AGENTS.md alone if no project AGENTS.md exists |
