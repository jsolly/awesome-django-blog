...
- **Cloud agents:** no developer-home config on the VM — everything comes from the repo subtree and project `AGENTS.md`. At cloud task start run `./scripts/cloud-fleet-sync-if-stale.sh` when fleet may have moved; pin the VM snapshot per `docs/cloud-agents.md` → **Snapshot bootstrap (agent-run)** (`./scripts/pin-cloud-snapshot.sh`).
